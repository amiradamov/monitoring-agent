...
- Preserve bounded log storage. Local logs use size-based rotation controlled by `local_log_max_bytes` and `local_log_backup_count`; avoid changes that can create unbounded local or server log growth.
- Preserve dashboard-oriented log fields such as IP age, boot time, uptime, and threshold-time top process snapshots. These fields are intended for future Telegram bot summaries and alerts.
...
- For logging changes, verify that the total local footprint remains bounded: approximately `(local_log_backup_count + 1) * local_log_max_bytes` per computer.
...
- Long-running installs depend on log rotation to avoid filling user disks and server storage.
...
- For the planned Telegram bot, prefer deriving online/offline state from the latest uploaded event timestamp and alert when a computer has not uploaded within the expected check interval plus grace period.
- Consider future Windows Event Log ingestion for shutdown, restart, sleep, wake, and unexpected power-loss events.
